...
namespace ogdf
{
		...
		EdgeArray<EdgeAttributes> &E)
	{
		...
		if (edgeLengthMeasurement() == elmMidpoint)
			forall_edges(e, G)
				E[e]
					.set_length(E[e].get_length() * unitEdgeLength());
		...
		else //(edgeLengthMeasurement() == elmBoundingCircle)
		{
			...
			forall_edges(e, G)
				E[e]
					.set_length(E[e].get_length() * unitEdgeLength() + radius[e->source()] + radius[e->target()]);
		}
	}
		...
		EdgeArray<EdgeAttributes> &E_reduced)
	{
		...
		forall_nodes(v_orig, G)
			A[v_orig]
				.set_copy_node(G_reduced.newNode());
		...
		forall_nodes(v_orig, G)
		{
			...
			A_reduced[v_reduced].set_NodeAttributes(A[v_orig].get_width(), A[v_orig].get_height(), A[v_orig].get_position(),
													v_orig, NULL);
		}
		forall_edges(e_orig, G)
		{
		...
		EdgeArray<double> &new_edgelength)
	{
		...
		forall_edges(e_act, G) if (E[e_act].get_copy_edge() != NULL) // e_act is no self_loops
	...
	void FMMMLayout::make_positions_integer(Graph &G, NodeArray<NodeAttributes> &A)
	{
		...
		forall_nodes(v, G) if ((A[v].get_x() > max_integer_position) ||
							   ...
							   (A[v].get_y() < max_integer_position * (-1.0)))
		{
			...
			else if (s.intersection(bottom_bound, cross_point))
			{
				...
				cout << "Error FMMMLayout:: make_positions_integer()" << endl;
		}
	...
	void FMMMLayout::create_postscript_drawing(GraphAttributes &AG, char *ps_file)
	{
		...
		forall_edges(e, G)
				out_fmmm
			<< AG.x(e->source()) << " " << AG.y(e->source()) << " "
			<< AG.x(e->target()) << " " << AG.y(e->target()) << " e" << endl;
		forall_nodes(v, G)
				out_fmmm
			<< AG.x(v) << " " << AG.y(v) << " v" << endl;
		...
		NodeArray<int> &component)
	{
		...
		forall_nodes(v_orig, G)
			A[v_orig]
				.set_subgraph_node(G_sub[component[v_orig]].newNode());
		forall_edges(e_orig, G)
		{
			u_orig = e_orig->source();
			v_orig = e_orig->target();
			E[e_orig].set_subgraph_edge(G_sub[component[u_orig]].newEdge(A[u_orig].get_subgraph_node(), A[v_orig].get_subgraph_node()));
		}
		...
		NodeArray<NodeAttributes> A_sub[])
	{
		...
		{ // allcomponents
			...
			for (j = 0; j <= stepsForRotatingComponents(); j++)
			{
				...
				forall_nodes(v_sub, G_sub[i])
				{
					new_pos.m_x = cos_j * old_coords[i][v_sub].m_x - sin_j * old_coords[i][v_sub].m_y;
					new_pos.m_y = sin_j * old_coords[i][v_sub].m_x + cos_j * old_coords[i][v_sub].m_y;
			...
			forall_nodes(v_sub, G_sub[i])
				A_sub[i][v_sub]
					.set_position(best_coords[i][v_sub]);
			R.pushBack(r_best);
		...
		NodeArray<NodeAttributes> A_sub[])
	{
		...
		{ // for
			...
			forall_nodes(v_sub, G_sub[i])
			{
				newpos = A_sub[i][v_sub].get_position() + r.get_new_dlc_position() - r.get_old_dlc_position();
		...
		NodeArray<DPoint> &F)
	{
		...
		forall_nodes(v, G)
			A[v]
				.set_position(A[v].get_position() + F[v]);
	}
